...
export function CsvUploader() {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      fd.append('file', file!);
      return apiUpload<{ jobId: string }>('/upload-products-csv', fd);
    },
    onSuccess: (d) => {
      toast.success('Product feed uploaded.');
    ...
    <div className="space-y-6">
      ...
      >
        ...
        {file ? (
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11.5px] text-muted-foreground">For bulk ingestion of listings</p>
          </div>
        )}
        <input
          type="file"
      ...
      <div className="space-y-2 rounded-xl border border-border bg-surface-muted/60 p-4">
        ...
        <p className="font-mono text-[12px] leading-relaxed text-muted-strong">
          sku_id, product_title, description, brand, category, price, mrp,
          availability
        </p>
      </div>
      ...
      >
        {mut.isPending ? 'Processing feed…' : 'Import bulk data'}
      </button>
    </div>
  );
}
